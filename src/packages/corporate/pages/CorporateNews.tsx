          ...
          <Link to={`/corporate/news/${lead.slug}`} className="group grid items-center gap-8 border-b border-line pb-12 lg:grid-cols-2">
            ...
            <div>
              <div className="flex items-center gap-3 text-xs text-ink-subtle">
                <span className="badge-accent">{lead.tag}</span><span>{formatDay(lead.publishedAt)}</span>
              </div>
        ...
        <div className="mt-12 grid gap-8 md:grid-cols-2 lg:grid-cols-3">
          {rest.map((n) => (
            <Link key={n.id} to={`/corporate/news/${n.slug}`} className="group flex flex-col">
              ...
              <div className="mt-4 flex items-center gap-3 text-xs text-ink-subtle"><span className="badge-neutral">{n.tag}</span><span>{formatDay(n.publishedAt)}</span></div>
